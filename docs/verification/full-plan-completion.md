...
- 工具：**按运行模式分别统计**（见 `capability-counts.json`）。Managed 158 条：136 L3、3 L4、19 L1，34 个域；Connector 在 Managed 之上增加 5 条可选 `official_uploader.*`：163 条、136 L3、3 L4、24 L1，35 个域。两种模式不合并为单一总数。
