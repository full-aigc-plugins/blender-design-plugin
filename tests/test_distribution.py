...
class TestManifestAndMarketplace(unittest.TestCase):
    ...
    def test_manifest_identity(self) -> None:
        ...
        self.assertRegex(manifest["version"], r"^0\.9\.0(?:\+codex\.[0-9A-Za-z.-]+)?$")
...
class TestValidatorRejectsDefects(unittest.TestCase):
    ...
    def test_rejects_corrupt_pinned_partme_runtime(self):
        target = self.repo / "vendor/partme-blender-mcp-runtime-0.3.0.zip"
    ...
    def test_accepts_cachebuster_build_suffix(self):
        """Local iteration requires 0.9.0+codex.<cachebuster>.
        ...
        self._mutate(
            self.manifest,
            lambda d: d.update(version="0.9.0+codex.local-20260919-120000"),
        )
