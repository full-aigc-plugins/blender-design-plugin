...
class TestManifestAndMarketplace(unittest.TestCase):
    ...
    def test_manifest_identity(self) -> None:
        ...
        self.assertRegex(manifest["version"], r"^0\.11\.3(?:\+codex\.[0-9A-Za-z.-]+)?$")
    ...
    def test_marketplace_pins_this_repository(self) -> None:
        ...
        self.assertEqual(
            entries[0]["source"],
            {"source": "url", "url": REPOSITORY + ".git", "ref": "v0.11.3"},
        )
        self.assertEqual(
            entries[0]["policy"],
...
class TestValidatorRejectsDefects(unittest.TestCase):
    ...
    def test_rejects_corrupt_pinned_partme_runtime(self):
        target = self.repo / "vendor/partme-blender-mcp-runtime-0.5.3.zip"
    ...
    def test_accepts_cachebuster_build_suffix(self):
        """Local iteration requires 0.11.3+codex.<cachebuster>.
        ...
        self._mutate(
            self.manifest,
            lambda d: d.update(version="0.11.3+codex.local-20260921-120000"),
        )
