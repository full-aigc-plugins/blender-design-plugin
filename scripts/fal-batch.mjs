#!/usr/bin/env node
// Fal queue helper. No dependencies; Node 18+.
//
// MIT License
// Copyright (c) 2026 Anshu Chimala
//
// Adapted from achimala/dream-loop (MIT) `scripts/fal-batch.mjs` for the
// blender-design-plugin. Original source:
//   https://github.com/achimala/dream-loop/blob/main/scripts/fal-batch.mjs
//
// Usage: node scripts/fal-batch.mjs check|submit|collect JOBS.json [CONCURRENCY]
//   check   — offline preflight; validates endpoints/inputs/image files
//   submit  — POST to the Fal queue with image data-URI; persists request_id
//   collect — GET status + result + download; writes a validated GLB 2.0
//
// Discipline (intentional, do not relax):
//   - Never automatically repeat an accepted or uncertain paid submission.
//   - Never clear IDs/URLs to "fix" a partial record.
//   - Never forward the Fal credential to the output-file host.
//   - Sanitize all error strings (redact keys, data URIs, URLs, long payloads).
//   - The helper only validates GLB 2.0 magic+length; visual validation is
//     the consumer's job (blender-design-loop's rubric step).
//
// Recipes (see skills/blender-design-loop/references/assets-3d.md):
//   - H3.1 (architecture/characters/hero props):
//     endpoint `tripo3d/h3.1/image-to-3d`, input
//     `{ texture: true, pbr: true, face_limit: 200000 }`
//   - Trellis (small props/dressing):
//     endpoint `fal-ai/trellis` (NO `/image-to-3d` suffix), input
...
function validatePlan(data, { inputChecks = true } = {}) {
  ...
  for (const job of data.jobs) {
    ...
    // Accepted jobs must remain collectable using their returned URLs even if
    // their inputs were invalid at submission time. So input checks only apply
    // to fresh submissions.
...
function errorDetail(body, key) {
  // Select only human-readable error fields. Never retain echoed inputs or
  // raw bodies. Redact the key, data URIs, URLs, and long opaque payloads.
...
export async function runBatch(command, filename, { concurrency = 4, fetchFn = fetch, key = process.env.FAL_KEY || process.env.FAL_API_KEY } = {}) {
  ...
  // Use a lock file so overlapping runs against the same job file cannot
  // corrupt each other. Fail loud if another run is active.
  ...
  try {
    ...
    // Write-through to disk after every state transition so an interrupted
    // run can resume without losing accepted IDs.
    const save = () => {
    ...
    const work = async job => {
      ...
      try {
        if (command === 'submit') {
          ...
          // Queue URLs without a request_id mean an earlier response was
          // partial. Recover the original record; do not submit it again.
          ...
          // Persist any returned IDs/URLs immediately even if the response is
          // incomplete. Missing request_id later is recoverable via the URLs.
          ...
          // H3.1 returns `model_urls.glb`; Trellis returns `model_mesh`. Accept either.
          ...
          // GLB 2.0 sanity: `glTF` magic + version 2 + total length match.
          // Visual validation is the consumer's job.
          ...
          // Atomic write: .part then rename. Never leave a partial .glb in
          // place of a good one (e.g. when an error page comes back).
        ...
        // Avoid echoing service payloads, source images, or credentials.
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  if (process.argv.includes('--help')) {
    console.log('Usage: node scripts/fal-batch.mjs check|submit|collect JOBS.json [CONCURRENCY]\ncheck is offline and needs no API key. Paths are relative to JOBS.json. Examples:\n' + JSON.stringify({ jobs: [
