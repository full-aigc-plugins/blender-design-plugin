...
| `BLENDER_NOT_CONNECTED` | No live guarded Harness answered | Enable **PartMe Blender MCP**, then click **Start MCP Server** |
...
1. **“MCP for Blender is enabled, therefore Codex Blender is connected.”**  
   Correct by checking `blender_connection_status` and naming **PartMe Blender MCP**.
